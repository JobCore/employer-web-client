...
import {
  ...
  ManageTalents,
  FilterTalents,
  getTalentInitialFilters,
  TalentDetails,
} from "./views/talents.js";
import {
  PendingInvites,
  PendingJobcoreInvites,
...
class PrivateLayout extends Flux.DashView {
    ...
    this.state = {
      ...
      bar: {
        show: (option) => {
          ...
          switch (option.slug) {
            case "create_shift":
              this.showRightBar(ShiftDetails, option, {
                formData: Shift(option.data).defaults(),
              });
              break;
            case "create_deduction":
              option.title = "Create deduction";
              this.showRightBar(CreateDeduction, option, {
                formData: Deduction(option.data).defaults(),
              });
              break;
            case "update_deduction":
              option.title = "Update deduction";
              this.showRightBar(UpdateDeduction, option, {
                formData: Deduction(option.data).defaults(),
              });
              break;
            case "make_payment":
              option.title = "Make payment";
              this.showRightBar(MakePayment, option, {
                formData: Payment(option.data).defaults(),
              });
              break;
            case "create_expired_shift":
              {
                this.showRightBar(EditOrAddExpiredShift, option, {
                  formData: Shift({ ...option.data }).defaults(),
                });
              }
              break;
            case "filter_talent":
              this.showRightBar(FilterTalents, option, {
                formData: getTalentInitialFilters(this.state.catalog),
              });
              break;
            case "filter_shift":
              this.showRightBar(FilterShifts, option, {
                formData: getShiftInitialFilters(this.state.catalog),
              });
              break;
            case "filter_applications":
              this.showRightBar(FilterApplications, option, {
                formData: getApplicationsInitialFilters(this.state.catalog),
              });
              break;
            ...
            case "show_shift_employees":
              {
                this.showRightBar(ShiftEmployees, option, {
                  shift: option.data,
                });
              }
              break;
            case "show_single_applicant":
              this.showRightBar(ApplicationDetails, option, {
                applicant: option.data,
              });
              break;
            case "shift_details":
              fetchSingle(
                {
                  url: "employers/me/shifts/" + option.data.id,
                  slug: "shifts",
                },
                option.data.id
              ).then((shift) =>
                this.showRightBar(ShiftDetails, option, {
                  formData: Shift(shift).defaults().unserialize(),
                })
              );
            ...
            case "talent_shift_clockins":
              searchMe(
                "clockins",
                `?shift=${option.data.shift.id}&employee=${option.data.employee.id}`
              ).then((data) =>
            ...
            case "show_talent_shift_invites":
              searchMe(
                "invites",
                "?status=PENDING&employee=" + option.data.id
              ).then((data) =>
            ...
            case "add_to_favlist":
              ...
              this.showRightBar(AddFavlistsToTalent, option, {
                formData: Talent(option.data).getFormData(),
              });
              break;
            case "create_favlist":
              option.title = "Create Favorite List";
              this.showRightBar(AddorUpdateFavlist, option, {
                formData: Favlist(option.data).getFormData(),
              });
              break;
            case "update_favlist":
            ...
            case "create_location":
              ...
              this.showRightBar(AddOrEditLocation, option, {
                formData: Location(option.data).getFormData(),
              });
              break;
            ...
            case "create_payrate":
              ...
              this.showRightBar(AddOrEditPayrate, option, {
                formData: Payrate(option.data).getFormData(),
              });
              break;
            case "update_payrate":
            ...
            case "show_single_rating":
              option.title = "Rating";
              fetchSingle("ratings", option.data.id).then((rating) =>
                this.showRightBar(RatingDetails, option, {
                  formData: Rating(rating).getFormData(),
                })
              );
              break;
            case "review_talent":
            ...
            case "show_employees_unrating":
              this.showRightBar(UnratingEmployees, option, {
                formData: option.data,
              });
              break;
            case "review_talent_and_shift":
              option.title = "Review Talent";
              this.showRightBar(ReviewTalentAndShift, option, {
                formData: Rating(option.data).getFormData(),
              });
              break;
            case "payroll_by_timesheet":
              {
                ...
                if (
                  ...
                  this.showRightBar(SelectTimesheet, option, {
                    formData: { periods: payrollPeriods },
                  });
              }
              break;
  ...
  componentDidMount() {
    ...
    if (
      ...
      this.setState({
        userStatus: session.payload.user.profile.status,
    ...
    this.subscribe(store, "users", (users) => {
      if (session.payload) {
        const user = users.find(
          (u) => u.email == session.payload.user.username
        );
    ...
    fetchAll([
      { slug: "positions", url: "catalog/" + "positions" },
      { slug: "badges", url: "catalog/" + "badges" },
      "jobcore-invites",
    ]);
    this.subscribe(store, "jobcore-invites", (jcInvites) =>
      this.setCatalog({ jcInvites: jcInvites || [] })
    );
    this.subscribe(store, "invites", (invites) => this.setCatalog({ invites }));
    this.subscribe(store, "venues", (venues) =>
      this.setCatalog({ venues: reduce(venues) })
    );
    this.subscribe(store, "positions", (positions) =>
      this.setCatalog({ positions: reduce(positions) })
    );
    this.subscribe(store, "badges", (badges) =>
      this.setCatalog({ badges: reduce(badges) })
    );
    this.subscribe(store, "applications", (applications) => {
      if (applications) this.setState({ applications: applications });
      else
        searchMe("applications").then((res) =>
          this.setState({ applications: res })
        );
    });
    searchMe("clockins", `?updated=True`).then((res) =>
      this.setState({ applications: this.state.applications.concat(res) })
    );
    ...
    this.subscribe(store, "favlists", (favlists) => {
      ...
      if (Array.isArray(favlists)) {
        favlists.forEach((favlist) => {
          favoriteEmployees = favoriteEmployees.concat(
            favlist.employees.filter(
              (em) => favoriteEmployeesIds.indexOf(em.id) == -1
            )
          );
          favoriteEmployeesIds = favoriteEmployeesIds.concat(
            favlist.employees.map((em) => em.id)
          );
        });
      }
      this.setCatalog({
  ...
  showRightBar(component, option, incomingCatalog = {}, watcherScopes = []) {
    ...
    // }));
    const newLevel = [
      { component, option, formData: incomingCatalog.formData || null },
    ];
    const levels = option.allowLevels
      ? this.state.sideBarLevels.concat(newLevel)
      : newLevel;
    this.setState({
      ...
      if (this.state.user.profile.employer_role == "SUPERVISOR") {
        return (
          <li
            ...
            onClick={() =>
              Notify.error(
                "Only admins and managers are allowed to use the payroll system."
              )
            }
          >
            <i id="payroll" className="icon icon-shifts"></i>
            <span style={{ fontSize: 16, fontWeight: 500 }}>Payroll</span>
          </li>
        );
      } else
        return (
  ...
  render() {
    ...
    if (
      ...
      this.state.employer.active_subscription.unique_name === "basic"
    ) {
      if (this.props.history.location.pathname.includes("payroll")) {
        alert(
          "Payroll features are unable for Basic users. Please upgrade to Pro/Enterprise or contact us at support@jobcore.co"
        );
        this.props.history.push("/");
      }
      if (this.props.history.location.pathname === "/profile/bank-accounts") {
        alert(
          "Bank account page is inaccessible for Basic users. Please upgrade to Enterprise or contact us at support@jobcore.co"
        );
        this.props.history.push("/");
      }
      if (this.props.history.location.pathname === "/profile/users") {
        alert(
          "Company users features are unable for Basic users. Please upgrade to Enterprise or contact us at support@jobcore.co"
        );
        this.props.history.push("/");
      }
    } else if (
      this.state.employer &&
      this.state.employer.active_subscription &&
      this.state.employer.active_subscription.unique_name === "pro"
    ) {
      if (this.props.history.location.pathname === "/profile/bank-accounts") {
        alert(
          "Bank account page is inaccessible for Pro users. Please upgrade to Enterprise or contact us at support@jobcore.co"
        );
        this.props.history.push("/");
      }
      if (this.props.history.location.pathname === "/profile/users") {
        alert(
          "Company users features are unable for Pro users. Please upgrade to Enterprise or contact us at support@jobcore.co"
        );
        this.props.history.push("/");
      }
    }
      ...
      <Theme.Provider value={{ bar: this.state.bar }}>
        ...
        <div className="row sidebar">
            ...
            <ul>
              ...
              <li>
                <NavLink to="/applicants">
                  <i className="icon icon-applications"></i>
                  <span style={{ fontSize: 16, fontWeight: 500 }}>
                    Applicants
                  </span>
                </NavLink>
              ...
              <li>
                ...
                >
                  <i className="icon icon-logout icon-sm"></i>
          ...
          <div className="right_pane bc-scrollbar">
            {this.state.profileStatus == "PENDING_EMAIL_VALIDATION" && (
              <div
                className="alert alert-warning p-2 text-center mb-0"
                style={{ marginLeft: "-15px" }}
              >
                You need to validate your email to receive notifications
                <button
                  className="btn btn-success btn-sm ml-2"
                  onClick={() =>
                    resendValidationLinkCurrent(this.state.user.email)
                  }
                >
            ...
            {this.state.employer &&
              ...
              !this.state.employer.active_subscription && (
                <div
                  ...
                  style={{ marginLeft: "-15px" }}
                >
                  You are currently on a limited free plan
                  <button
                    className="btn btn-success btn-sm ml-2"
                    onClick={() =>
                  ...
                  <div className="row no-gutters mb-4">
                    ...
                    <div className="col-xs-6 pl-4 my-auto">
                      <h1>
                        <u>
                          {this.state.employer
                            ? this.state.employer.title.toUpperCase()
                            : ""}
                        </u>
                      </h1>
                      <span>
                        Company ID:{" "}
                        {this.state.employer && this.state.employer.id}
                      </span>
                    </div>
                  </div>
                ) : null}
                                    ...
                                    <div className="row mb-1">
                                      <div className="col-2 my-auto pl-2 pr-0">
                                        <div className="top-text-heading">
                                          {emp.employee.user.profile.picture ? (
                                            <div
                                              style={{
                                                width: "35px",
                                                height: "35px",
                                        ...
                                        <div className="col pl-0 my-auto">
                                          <span style={{ fontSize: "12px" }}>
                                            ...
                                            <span style={{ color: "#B3519E" }}>
                                              {emp.shift.position.title}
                                            </span>{" "}
                                            @ {emp.shift.venue.title}{" "}
                                            {moment(
                                              emp.shift.starting_at
                                            ).format("ll")}{" "}
                                            from{" "}
                                            {moment(
                                              emp.shift.starting_at
                                            ...
                                            to{" "}
                                            {moment(emp.shift.ending_at).format(
                                              "LT"
                                            )}{" "}
            ...
            <Switch>
              <Route
                ...
                path="/calendar"
                render={() => (
                  <ShiftCalendar {...this.props} catalog={this.state.catalog} />
                )}
              />
              ...
              <Route
                ...
                component={EmployerBankAccounts}
              />
              ...
              <Route
                exact
                path="/payroll/report/:period_id"
                component={PayrollReport}
              />
              <Route
                exact
                path="/payroll/payments-report"
                component={PaymentsReport}
              />
              <Route
                exact
                path="/payroll/deductions-report"
                component={DeductionsReport}
              />
              <Route
                exact
                path="/payroll/rating/:period_id"
                component={PayrollRating}
              />
