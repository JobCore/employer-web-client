...
const gf = {
  positions: {
    ...
    label: (s) => (
      <span>
        <ShiftBadge {...s} /> {s.venue.title || s.position.label}
      </span>
    ),
  },
  venues: {
    ...
    label: (s) => (
      <span>
        <ShiftBadge {...s} /> {s.position.title || s.position.label}
      </span>
    ),
  },
  employees: {
...
export const ShiftCalendar = ({ catalog }) => {
  ...
  const setCalendarFilters = (incoming = {}) => {
    ...
    if (moment.isMoment(_filters.start))
      _filters.start = _filters.start.format("YYYY-MM-DD");
    if (moment.isMoment(_filters.end))
      _filters.end = _filters.end.format("YYYY-MM-DD");
    searchMe(
      "shifts",
      "?limit=10000&" + queryString.stringify(_filters) + "&serializer=big"
    );
  };
  const groupShifts = (sh, l = null) => {
    ...
    if (l === null || l.value === "employees") {
      ...
      employees.forEach((emp) => {
        _shifts[
          ...
          .filter((s) => s.employees.find((e) => emp.id === e.id))
          .map((s) => ({
      ...
      _keys.forEach((key) => {
        if (Array.isArray(_shifts[key]))
          _shifts[key] = _shifts[key].map((s) => ({
            start: moment(s.starting_at),
            end: moment(s.ending_at),
    ...
    else if (groupedLabel === null)
      setGroupedLabel({ label: "Position", value: "positions" });
  };
  ...
  useEffect(() => {
    ...
    if (previousLabel.current !== groupedLabel)
      previousLabel.current !== groupedLabel;
    ...
    const unsubscribeVenues = store.subscribe("venues", (venues) =>
      setVenues(venues)
    );
    const unsubscribePositions = store.subscribe("positions", (positions) =>
          ...
          <div className="col-10">
            {groupedShifts && (
              <CalendarView
                        ...
                        onClick={() => {
                          ...
                          if (newEndDate.isBefore(oldEndDate)) {
                            ...
                            const updatedFilters = {
                              start: moment(newEndDate)
                                .add(-2, "weeks")
                                .format("YYYY-MM-DD"),
                              end: moment(newEndDate)
                                .add(2, "weeks")
                                .format("YYYY-MM-DD"),
                            };
                            window.location.hash =
                              queryString.stringify(updatedFilters);
                ...
                onClick={(e) => {
                  const venue =
                    groupedLabel.value === "venues"
                      ? venues.find((v) => v.title == e.yAxis)
                      : undefined;
                  const position =
                    groupedLabel.value === "positions"
                      ? positions.find(
                          (p) => p.label == e.yAxis || p.title == e.yAxis
                        )
                      : undefined;
                  const employee = shifts
                    .filter((emp) => emp.employees.length > 0)
                    .map((_emp) => _emp.employees)
                    .flat()
                    .find(
                      (employee) =>
                      ...
                      data: {
                        ...
                        position:
                          position && position.id
                            ...
                            ? position.value
                            : "",
                        application_restriction: employee
                          ? "SPECIFIC_PEOPLE"
                          : "ANYONE",
                        pending_invites: employee
                          ? [
                              {
                                label:
                                  employee.user.first_name +
