...
class ButtonBar extends React.Component {
      ...
      buttonBarActions: {
        home: [
          { slug: "create_shift", title: "Create shifts", to: "shifts" },
          {
            ...
            title: "Invite Talent to JobCore",
            to: "talents",
          },
        ],
        locations: [
        ...
        "payroll/*": [
          {
            slug: "payroll_by_timesheet",
            title: "Search by Timesheet",
            to: "/payroll",
          },
          {
            slug: "payroll_settings",
            title: "Payroll Settings",
            to: "/payroll/settings",
          },
          {
            slug: "payroll_payments_report",
            title: "Payments Report",
            to: "/payroll/payments-report",
          },
          {
            slug: "payroll_deductions_report",
            title: "Deductions Report",
            to: "/payroll/deductions-report",
          },
  ...
  getMatchingExpression(path, actions) {
    ...
    for (let expression in actions)
      if (path.match(expression)) return expression;

    return null;
  }

  componentDidMount() {
    const key = this.getMatchingExpression(
      this.props.history.location.pathname,
      this.state.buttonBarActions
    );
  ...
  render() {
    const buttons = this.state.currentButtons.map((btn, i) => (
      <li key={i}>
        <button
          id={btn.slug}
