...
export default class ShiftCard extends React.Component{
    ...
    render(){
        ...
        const startDate = moment(this.props.shift.starting_at).format('ll');
        const startTime = moment(this.props.shift.starting_at).format('LT');
        const endTime = moment(this.props.shift.ending_at).format('LT');
                    ...
                    <div className="shift-details">
                        ...
                        <span className="shift-date"> {startDate} from {startTime} to {endTime} </span>
                        {
                            (typeof this.props.shift.price == 'string') ?
                                (this.props.shift.price === '0.0') ? '': <span className="shift-price"> ${this.props.shift.price ? this.props.shift.price : this.props.shift.minimum_hourly_rate}</span>
                            :
                                <span className="shift-price"> {this.props.shift.price ? this.props.shift.price.currencySymbol : "$"}{this.props.shift.price ? this.props.shift.price.amount : this.props.shift.minimum_hourly_rate}</span>
                        }
